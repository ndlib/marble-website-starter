...
exports.sourceNodes = async (
  ...
  const { url, key, website, itemList, debug = false } = pluginOptions
  const { cache, actions } = gatsbyInternal
  ...
  if (cachedMarbleNodes) {
    ...
    await cache.set('marbleNodes', nodeArray)

    if (debug) {
      const cleanNodeData = nodeArray.map(n => {
